...
const FooterMain = <T extends Object>({
  ...
  return (
    ...
    >
      ...
      {columnsHeaderStructure[StructureConfig.BB55].items.map(
        (item: IColumnHeader, index: number) =>
          //@ts-ignore
          columnsConfigStructure[StructureConfig.BB55].items.map(
              ...
              if (index === indexx) {
                return (
                  column.isVisible && (
                    ...
                    >
                      {column.footer?.(
                        columnsHeaderStructure[StructureConfig.BB55].items[
                          index
                        ] as any
                      )}
                    </li>
