...
const FooterMain = <T extends Object>({
  ...
  return (
    <ul
      ...
      style={{ flex: 1, backgroundColor: footerColor && footerColor }}
    >
      {!isStickyFirstColumn ? (
        <li
      ...
      {columnsHeaderStructure[StructureConfig.Main].items.map(
        (item: IColumnHeader, index: number) =>
          //@ts-ignore
          columnsConfigStructure[StructureConfig.Main].items.map(
            (column: IColumnConfig<T>, indexx: number) => {
              if (index === indexx) {
                return (
                  column.isVisible && (
                    <li
                    ...
                    >
                      {column.footer?.(
                        columnsHeaderStructure[StructureConfig.Main].items[
                          index
                        ] as any
                      )}
                    </li>
