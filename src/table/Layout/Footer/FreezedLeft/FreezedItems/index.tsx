...
const FreezedItems = <T extends Object>({
  ...
  return (
    <>
      {columnsHeaderStructure[StructureConfig.BB33].items.map(
        (item: IColumnHeader, index: number) =>
          //@ts-ignore
          columnsConfigStructure[StructureConfig.BB33].items.map(
              ...
              if (index === indexx) {
                return (
                  column.isVisible && (
                    ...
                    >
                      {column.footer?.(
                        columnsHeaderStructure[StructureConfig.BB33].items[
                          index
                        ] as any
                      )}
                    </li>
